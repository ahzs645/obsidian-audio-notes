...
__export(Transcript_exports, {
  ...
  getTranscriptFromDGResponse: () => getTranscriptFromDGResponse,
  getTranscriptFromScriberrResponse: () => getTranscriptFromScriberrResponse,
  getYouTubeTranscript: () => getYouTubeTranscript,
  parseTranscript: () => parseTranscript
});
module.exports = __toCommonJS(Transcript_exports);
...
var Transcript = class {
  ...
  getQuote(quoteStart, quoteEnd) {
    ...
    if (start === void 0 || end === void 0) {
      new import_obsidian.Notice("Transcript file does not have start or end times for at least one text entry.");
...
function parseTranscript(contents) {
  ...
  try {
    ...
    if (parsed == null ? void 0 : parsed.segments) {
      return new Transcript(parsed.segments);
    }
  } catch (e) {
  }
  if (/^WEBVTT(?=$|\s)/i.test((_a = trimmed.split("\n")[0]) != null ? _a : "")) {
    try {
...
function getTranscriptFromDGResponse(response) {
  ...
  const segments = [new TranscriptSegment(0, firstWord.start, firstWord.start, firstWord.punctuated_word)];
  let id = 1;
  for (let i = 1; i < bestAlternative.words.length - 2; i++) {
    const word = bestAlternative.words[i];
    const lastSegment2 = segments[segments.length - 1];
    lastSegment2.text += " " + word.punctuated_word;
    lastSegment2.end = word.end;
...
function getTranscriptFromScriberrResponse(response) {
  const segments = response == null ? void 0 : response.segments;
...
function normalizeScriberrSegment(segment, defaultId) {
  var _a;
  if (!segment) {
    return void 0;
  }
  ...
  const end = parseNumber((_a = segment.end) != null ? _a : start);
...
function getYouTubeTranscript(url) {
  return __async(this, null, function* () {
    const html = yield (0, import_obsidian.request)({
      url,
      method: "GET"
    });
    function unescapeText(s) {
      var re = /&(?:amp|#38|lt|#60|gt|#62|apos|#39|quot|#34);/g;
      var unescaped = /* @__PURE__ */ new Map([
        ["&amp;", "&"],
        ["&#38;", "&"],
        ["&lt;", "<"],
        ["&#60;", "<"],
    ...
    for (const capmeta of captionsMetadata) {
      if (capmeta.languageCode === "en") {
        const xml = yield (0, import_obsidian.request)({
          url: capmeta.baseUrl,
          method: "GET"
        });
        const xmlParser = new import_fast_xml_parser.XMLParser({ ignoreAttributes: false });
        ...
        for (const line of parsed) {
          ...
          text = text.replace(sentenceCaseRegex, function(toReplace) {
...
var WebVttParser = class {
  fromVtt(data) {
    ...
    const headerStripped = sanitized.replace(/^WEBVTT[^\n]*\n+/i, "").trim();
    ...
    for (const block of blocks) {
      ...
      const endRaw = endRawWithSettings == null ? void 0 : endRawWithSettings.trim().split(/\s+/)[0];
      if (!startRaw || !endRaw) {
        continue;
      }
  ...
  parseTimestamp(value) {
    ...
    const secondsPart = (_a = parts.pop()) != null ? _a : "0";
    const minutesPart = (_b = parts.pop()) != null ? _b : "0";
    const hoursPart = (_c = parts.pop()) != null ? _c : "0";
    const [secondsString, millisString = "0"] = secondsPart.split(".");
    const seconds = parseInt(secondsString, 10) || 0;
    const milliseconds = parseInt(millisString.padEnd(3, "0").slice(0, 3), 10) || 0;
  ...
  parseCueText(lines) {
    ...
    if (voiceMatch) {
      speakerLabel = (_a = voiceMatch[1]) == null ? void 0 : _a.trim();
    ...
    return {
      text,
      speakerLabel,
      speakerId: speakerLabel ? this.normalizeSpeakerId(speakerLabel) : void 0
    };
  }
  normalizeSpeakerId(label) {
    return label.toLowerCase().normalize("NFKD").replace(/[^\w]+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "") || "speaker";
  }
};
var SrtParser = class {
  constructor() {
  ...
  fromSrt(data) {
    ...
    for (var i = 0; i < data_array.length; i += 4) {
      ...
      text = text.replace(/\n/, " ");
      const segment = new TranscriptSegment(data_array[i].trim(), this.timestampToSeconds(startTime), this.timestampToSeconds(endTime), text);
      segments.push(segment);
    }
    return new Transcript(segments);
  }
};
var TranscriptsCache = class {
  constructor(settings, loadFiles) {
    this.settings = settings;
    this.loadFiles = loadFiles;
    this.cache = /* @__PURE__ */ new Map();
  }
  getTranscript(transcriptFilename) {
    return __async(this, null, function* () {
      if (transcriptFilename === void 0) {
        return void 0;
      }
      if (this.cache.has(transcriptFilename)) {
        return this.cache.get(transcriptFilename);
      }
      let transcriptContents = void 0;
      let transcript = void 0;
      const isRemote = transcriptFilename.includes("://");
      if (!isRemote) {
        const translationFilesContents = yield this.loadFiles([transcriptFilename]);
        transcriptContents = translationFilesContents.get(transcriptFilename);
        if (transcriptContents !== void 0) {
        ...
        const urlParams = /* @__PURE__ */ new Map();
        for (const param of urlParts[1].split("&")) {
          const [key, value] = param.split("=");
          urlParams.set(key, value);
        }
        const url = `${urlParts[0]}?v=${urlParams.get("v")}`;
        transcript = yield getYouTubeTranscript(url);
      }
      if (transcript === void 0 && this.settings.audioNotesApiKey) {
        transcriptContents = yield (0, import_obsidian.request)({
